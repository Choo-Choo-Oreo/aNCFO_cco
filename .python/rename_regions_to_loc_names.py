"""Rename map/strategicregions/*.txt files to match their in-game localized name.

Each strategic region file is named ID-Descriptor.txt or ID.txt. This script
reads the file's `name = "KEY"` field, resolves KEY by scanning
strategic_region_names_l_english.yml under localisation/english/, and renames
the file to ID-<resolved name>.txt via `git mv` so the rename is tracked.
"""
...
STRATEGIC_REGIONS_DIR = REPO_ROOT / "map" / "strategicregions"
...
def load_loc_keys(loc_dir: Path) -> dict[str, str]:
    ...
    for loc_file in sorted(loc_dir.glob("strategic_region_names_l_english.yml")):
...
def main() -> int:
    if not STRATEGIC_REGIONS_DIR.is_dir():
        print(f"Strategic regions directory not found: {STRATEGIC_REGIONS_DIR}")
        return 1
    ...
    for path in sorted(STRATEGIC_REGIONS_DIR.glob("*.txt")):
        filename_match = FILENAME_RE.match(path.name)
        if not filename_match:
            unrecognized.append(path.name)
            continue
        region_id = filename_match.group(1)
        ...
        new_name = f"{region_id}-{sanitized}.txt"
    ...
    if unrecognized:
        print("\nUnrecognized filenames (didn't match ID.txt or ID-Descriptor.txt):")
